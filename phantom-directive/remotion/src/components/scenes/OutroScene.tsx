...
import {
  AbsoluteFill, Img, OffthreadVideo, interpolate, spring, useCurrentFrame, useVideoConfig,
} from 'remotion';
...
// Optional subtle media background behind the outro design (18% opacity)
// OffthreadVideo — proxied by Remotion internally, no cross-origin issues
const OutroMediaBg: React.FC<{ url?: string | null; mediaType?: string | null }> = ({ url, mediaType }) => {
...
export const OutroScene: React.FC<{ data: NewsCardData }> = ({ data }) => {
  ...
  return (
    <AbsoluteFill style={{ background: THEME.bg, opacity: oExit }}>
      {/* Optional subtle media background at low opacity */}
      <OutroMediaBg url={data.mediaUrl} mediaType={data.mediaType} />
      ...
      <div style={{
        ...
        }}>Phantom Directive · Classified History</span>
        <span style={{
