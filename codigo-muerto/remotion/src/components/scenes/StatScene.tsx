...
export const StatScene: React.FC<{ data: NewsCardData }> = ({ data }) => {
  ...
  const iconO    = interpolate(frame, [16, 34], [0, 1],   { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const iconS    = spring({ frame: frame - 16, fps, from: 0.4, to: 1, durationInFrames: 28, config: { damping: 11 } });
    ...
    <AbsoluteFill style={{ background: THEME.bg, opacity: oExit }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 900 }}>
          ...
          <div style={{ opacity: textO, maxWidth: 860 }}>
            <p style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            background: `radial-gradient(circle, ${hex(color, 0.16)} 0%, transparent 65%)`,
            pointerEvents: 'none',
          }} />
          <div style={{
            position: 'absolute', inset: -8, borderRadius: '50%',
            border: `2px solid ${hex(color, 0.45)}`,
            boxShadow: `0 0 30px ${hex(color, 0.18)}`,
          }} />
          <IconComp name={data.icon} size={110} />
        </div>
      </div>
    </AbsoluteFill>
  );
